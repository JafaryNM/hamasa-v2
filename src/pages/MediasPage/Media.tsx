...
export default function Media() {
  ...
  const fields =
    data?.results?.map((field: MediaData, index: number) => ({
      ...field,
  ...
  const openEditModal = (record: MediaData) => {
    reset({
      name: record.name || "",
  ...
  const handleConfirmDelete = () => {
    ...
    deleteMedia.mutate(recordToDelete.id, {
      ...
      onError: (err: any) => {
        toast.error(
          err?.response?.data?.detail || "Failed to delete media source"
        );
      },
    });
  };
    ...
    if (editingRecord) {
      updateMedia.mutate(
        { id: editingRecord.id!, data: values },
        {
          ...
          onError: (err: any) =>
            toast.error(
              err?.response?.data?.detail || "Failed to update media source"
            ),
        }
      );
    } else {
      addMedia.mutate(values, {
        onSuccess: () => {
          toast.success("Media source created successfully");
          setIsModalOpen(false);
        },
        onError: (err: any) =>
          toast.error(
            err?.response?.data?.detail || "Failed to create media source"
          ),
      });
    }
  };
